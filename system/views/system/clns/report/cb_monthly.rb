...
Clns::CacheBook.monthly(*date).asc(:id_date).each do |cbd|
  pdf.start_new_page
  pdf.font 'Verdana'
  pdf.font_size = 8
  ...
  pdf.move_down 15
  pdf.text "Registru de casă pagina: #{cbd.name} din date de #{cbd.id_date.to_s}",
    size: 11
  pdf.move_down 15
  pdf.table table_data(cbd), cell_style: {border_width: 0.1, padding: [2,5,2,5], size: 9}, column_widths: [10.mm,30.mm,100.mm,20.mm,20.mm,10.mm] do
  ...
  pdf.move_down 15
  pdf.table table_recap(cbd),cell_style: {border_width: 0, padding: [2,5], size: 9}, column_widths: [30.mm, 30.mm, 40.mm, 30.mm], position: :center do
